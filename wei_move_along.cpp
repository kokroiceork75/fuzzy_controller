...
class controller
{
	public:
		...
		{	
		    ...
		    else if(status == 2 && (straight_min <= f_along || left_min <= s_along))// && laser_temp[90] < 0.7)// && decision_left == 0  )
			{	
			 	decision_right = 0;
				decision_left  = 1;
			
			}
			else if(status == 3 && (straight_min <= f_along || right_min <= s_along))// && laser_temp[90] < 0.7)// && decision_left == 0  )
			{	
			 	decision_right = 1;
				decision_left  = 0;
		...
		void test_run(int j)
		{
			...
			if((v1-v2)/dis > 1.3 )  msg.angular.z = 1.3; 
			else if ((v1-v2)/dis <-1.3) msg.angular.z =-1.3;
