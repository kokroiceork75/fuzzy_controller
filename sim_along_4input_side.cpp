//////////////////////////////  4 input and the last input is side 310~325degree  ///////////////////////////////////////////
#include <sstream>
#include <iostream>
#include <algorithm>
#include "ros/ros.h"
...
////////////////////////////////////////////  Constructor  //////////////////////////////////////////////	
		controller(){
    		...
    		pub=n.advertise<geometry_msgs::Twist>("/cmd_vel_2",3000); //using fuzzy controller as navigate need to remap to cmd_vel_2
   ...
   {  
	  ...
	  return laser_min;
		...
		void decision_(int j){	
			...
			if (stright_min<=0.4 || left_min<=0.54 || right_min<=0.54){
				if (left_min<=0.54){
					decision_left = 1;
					decision_right= 0;
				}
				else if (right_min<=0.54){
					decision_left = 0;
					decision_right= 1;
				
				}
			    else if (left_min<right_min && left_min <0.54){
					decision_left = 1;
					decision_right= 0;
				}
				else if (left_min>right_min && right_min<0.54){
					decision_left = 0;
					decision_right= 1;
				}
			}
		    ...
		    else if(right_min < 0.54 && laser_temp[270] > 0.54 )// && laser_temp[90] < 0.7)// && decision_left == 0  )
			{	
			 	decision_right = 1;
				decision_left  = 0;
			
			}
			else if(left_min <0.54 && laser_temp[90]> 0.54)// && laser_temp[270] < 0.7 )//&& decision_right == 0 )
		    {
				decision_left = 1;
				decision_right= 0;
...
////////////////////////////////////  Output cmd_velocity  ////////////////////////////////////
		void test_run(int j){
			...
			if(decision_left == 1 ){
			v1 = out_y[1]*0.03*0.7;  // 0.03 is radius of wheel
			v2 = out_y[2]*0.03*0.7;
			}
			if(decision_right == 1 ){
			 v1 = out_y[2]*0.03*0.7;
